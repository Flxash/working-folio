...
export const ThumbnailModal = forwardRef(
  ...
  function ThumbnailModal({ variants, active, index, ...props }, ref) {
    const items = thumbnailOptions.map(({ title, image }, idx) => {
      const id = randomId();
      return (
        <Center
          key={id}
          className={`h-full w-full bg ${idx === 0 ? 'bg-slate-700' : 'bg-orange-100'}`}
        >
          <Image
            src={image}
            width={260}
            ...
            className='rounded-lg' // Add border radius directly to the image
          />
        </Center>
      );
    });
